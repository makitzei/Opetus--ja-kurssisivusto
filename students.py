...
def join(student_id,course_id):
    try:
        sql = "INSERT INTO student_course(student_id, course_id) VALUES (:student_id, :course_id) "\
            "ON CONFLICT ON CONSTRAINT ux_student_course DO NOTHING"
...
def get_courses(student_id):
    sql = "SELECT courses.id, courses.title "\
        "FROM courses JOIN student_course ON courses.id = student_course.course_id "\
        "JOIN users ON student_course.student_id = users.id WHERE users.id = :student_id"
